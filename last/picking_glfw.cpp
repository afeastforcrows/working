...
/////////////////////////////TRACKBALL STATE////////////////////////////
	...
	//{
		Vec3 oldPosition = (mousePosition);
		mousePosition = (mouseCurrentPosition( ), 0);
		mousePosition[0] = ((mousePosition[0]*2)/500)-1;
		mousePosition[1] = ((mousePosition[1]*2)/500)-1;
		//mousePos = Vec3(mousePosition[0], mousePosition[1], 0);
		...
		if(mouseButtonFlags( ) == GLFWApp::MOUSE_BUTTON_LEFT){	
			...
			{
				Vec3 axis = cross(mousePosition-Vec3(0,0,0.5),oldPosition-Vec3(0,0,0.5));
				float angle = 200;//atan(radiansToDegrees(dot(mousePos-Vec3(0,0.5,0),oldPosition-Vec3(0,0.5,0))));
				Mat3 rot = rotate3(angle, axis[0], axis[1], axis[2]);
				centerPosition = rot * centerPosition;
